...
import { DocumentSymbol, Location, Position, Range, SymbolInformation, SymbolKind, Uri, workspace } from 'vscode'
import { getSymbolData, isSupportedFile, isSupportedLanguage } from '../src/symbols.js'
...
suite('symbol classification', () => {
  ...
  test('recognizes supported language ids and file extensions', () => {
    ...
    assert.equal(isSupportedLanguage('typescriptreact'), true)
    assert.equal(isSupportedLanguage('javascript'), true)
    assert.equal(isSupportedLanguage('javascriptreact'), true)
    assert.equal(isSupportedLanguage('json'), false)

    for (const extension of supportedExtensions) {
      assert.equal(isSupportedFile(Uri.file(`/workspace/file.${extension}`)), true)
    }

    assert.equal(isSupportedFile(Uri.file('/workspace/file.json')), false)
  })
})
