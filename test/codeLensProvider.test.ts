...
suite('ZeroReferenceCodeLensProvider', () => {
  ...
  test('emits a CodeLens refresh after analysis invalidation', async () => {
    const fixture = await createSymbolFixture(1)
    const provider = createProvider(fixture.symbols, () => [])
    let refreshCount = 0
...
function createProvider(
  ...
  return new TestCodeLensProvider(analyzer)
}

class TestCodeLensProvider extends ZeroReferenceCodeLensProvider {
  constructor(private readonly testAnalyzer: ZeroReferenceAnalyzer) {
    super(testAnalyzer, () => true)
  }

  refresh(): void {
    this.testAnalyzer.invalidateGraph()
  }

  override dispose(): void {
    super.dispose()
    this.testAnalyzer.dispose()
  }
}
